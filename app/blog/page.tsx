...
export default function BlogPage() {
  ...
  useEffect(() => {
    supabase.from('page_content').select('*').eq('page_key', 'blog').then(({ data }) => {
      if (data) {
  ...
  return (
    <div style={{ background: '#0A0A0A', minHeight: '100vh', paddingTop: '64px' }}>
      <div style={{ padding: '80px 48px', maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '2.5rem', marginBottom: '16px', color: '#F0EDE8' }}>{content.title || 'STORIES & JOURNAL'}</h1>
        {content.subtitle && <p style={{ color: '#555', marginBottom: '64px' }}>{content.subtitle}</p>}
        <div style={{ display: 'grid', gap: '40px' }}>
          {DEFAULT_POSTS.map(post => (
            <div key={post.title} style={{ borderTop: '1px solid #1a1a1a', paddingTop: '40px', display: 'grid', gridTemplateColumns: '1fr 3fr', gap: '40px' }}>
              ...
              <div>
                <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.4rem', marginBottom: '16px', lineHeight: '1.4', color: '#F0EDE8' }}>{post.title}</h2>
